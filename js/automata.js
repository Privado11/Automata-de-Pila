...
function procesarEntrada(evento, entrada) {
  ...
  for (let i = 0; i < entrada.length; i++) {
    ...
    diagram.links.each(function (link) {
      //console.log('donde viene: '+link.data.from)
      //console.log('donde va: '+link.data.to)
      if(i<mitad && link.data.from==='0' && link.data.to==='0'){
        console.log('entro')
      ...
      }else if(i>=mitad && link.data.from==='0' && link.data.to==='1'){
        console.log('entro2')
...
function obtenerTope() {
  ...
  let tope = cantidad == 1 ? elementosPila[0].querySelector('p').textContent : elementosPila[0].textContent
